...
public class RenameTypeRefactoring extends RenameRefactoring<FeatureHouseClassSignature> {

	public RenameTypeRefactoring(FeatureHouseClassSignature selection, IFeatureProject featureProject, String file) {
	...
	private RefactoringStatus checkTypesInCompilationUnit(final SignatureMatcher matcher) {
		...
		final AbstractClassSignature classSignature = (AbstractClassSignature) selectedSignature;
		// 1. member types
		for (final AbstractClassSignature memberClass : classSignature.getMemberClasses()) {
		...
		// 2. Methods
		for (final AbstractMethodSignature methodSignature : classSignature.getMethods()) {
		...
		// 3. imports
		for (final String imported : classSignature.getImportList()) {
			if (imported.endsWith(".*;")) {
				final String packageName = imported.substring(7, imported.length() - 3);

				for (final AbstractSignature newMatchedSignature : matcher.getMatchedSignaturesForNewName()) {
					final AbstractClassSignature parent = newMatchedSignature.getParent();
					if ((parent != null) && (parent.equals(selectedSignature))) {
						continue;
					}

					if ((newMatchedSignature instanceof FeatureHouseClassSignature) && newMatchedSignature.isPublic()
		...
		for (final AbstractSignature newMatchedSignature : matcher.getMatchedSignaturesForNewName()) {
			...
			if ((newMatchedSignature instanceof FeatureHouseClassSignature) && RefactoringUtil.hasSamePackage(selectedSignature, newMatchedSignature)) {
				final FOPFeatureData[] featureData = (FOPFeatureData[]) newMatchedSignature.getFeatureData();
				for (int i = 0; i < featureData.length; i++) {
					if (selectedFeatureData.getID() == featureData[i].getID()) {
						result.addError(Messages.format(RefactoringCoreMessages.RenameTypeRefactoring_exists, new String[] {
							BasicElementLabels.getJavaElementName(newName), ((FeatureHouseClassSignature) newMatchedSignature).getPackage() }));
					}
				}
			}
		}
	...
	private boolean isMainMethod(AbstractMethodSignature method) {
		if ("main".equals(method.getName()) && method.getReturnType().equals("void") && method.isPublic() && method.isStatic()) {
			final List<String> paramTypes = method.getParameterTypes();
	...
	public RefactoringStatus checkNewElementName(String newName) {
		...
		final RefactoringStatus result = checkTypeName(newName);
