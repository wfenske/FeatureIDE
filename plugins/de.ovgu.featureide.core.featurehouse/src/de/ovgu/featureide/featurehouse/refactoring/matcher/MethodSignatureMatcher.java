...
public class MethodSignatureMatcher extends SignatureMatcher {
	...
	protected Set<AbstractSignature> determineMatchedSignatures() {
		...
		if (RefactoringUtil.isVirtual((FeatureHouseMethodSignature) selectedSignature)) {
	...
	private void filterMatchedSignatures(final Set<AbstractClassSignature> involvedClasses, final Set<AbstractSignature> matchedSignatures) {
		for (final AbstractSignature matchedSignature : new HashSet<>(matchedSignatures)) {
	...
	private void addSubClasses(final Set<AbstractClassSignature> involvedClasses, final Set<AbstractClassSignature> matchedClasses) {
		for (final AbstractClassSignature abstractClassSignature : matchedClasses) {
	...
	protected boolean hasSameType(AbstractSignature signature) {
		return (signature instanceof FeatureHouseMethodSignature);
	}
	...
	protected boolean checkSignature(final AbstractSignature signature, final String sigName) {
		return super.checkSignature(signature, sigName)
			&& RefactoringUtil.hasSameParameters((FeatureHouseMethodSignature) signature, (FeatureHouseMethodSignature) selectedElement)
			&& hasSameReturnType((FeatureHouseMethodSignature) signature);
	}
	...
	private void filterSuperClasses(final Set<AbstractClassSignature> involvedClasses, final Set<AbstractSignature> matchedSignatures) {
		for (final AbstractClassSignature involvedClass : new HashSet<>(involvedClasses)) {
			final AbstractSignature matchedSignature = getMatchedSignature(involvedClass, matchedSignatures);
	...
	private AbstractSignature getMatchedSignature(final AbstractClassSignature signature, final Set<AbstractSignature> matchedSignatures) {
		for (final AbstractSignature match : matchedSignatures) {
	...
	private boolean hasSameReturnType(final FeatureHouseMethodSignature signature) {
		return signature.getReturnType().equals(((FeatureHouseMethodSignature) selectedElement).getReturnType());
	}
